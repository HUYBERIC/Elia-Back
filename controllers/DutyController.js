...
const getDuties = async (req, res) => {
  try {
    ...
    .populate({
      path: "replacements", // On peuple le champ `replacements`
      populate: {
        path: "replacedUserId replacingUserId", // On peuple aussi les utilisateurs à l'intérieur de `replacements`
        model: "User", // Assure-toi que le modèle "User" est correct
      },
    });
...
const deleteDuty = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur lors de la suppression de la garde", error });
  }
};
